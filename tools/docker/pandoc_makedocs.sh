...
OUTPUT_BASE_NAME="OWASP_ISVS-${VERSION}"
...
echo "processing image tags in $FOLDER/V*.md"
for FILE in $FOLDER/V*.md 
do
...
if [ $LANGUAGE == "fa" ]; then
  pandoc --resource-path=.:${FOLDER} \
    ...
    --metadata title="OWASP IoT Security Verification Standard $VERSION" \
